...
- 快速为一个项目创建默认两 pane 的 agent 工作窗口，默认横向 55/45；可选第三个 run pane，并可显式开启随 resize 自动切换朝向。
...
- 按 **Settings → Window & Resize** 自动搭 ai/git（默认）或 ai/git/run 布局，在 ai pane 起选定客户端（如 `claude-minimax`），并把所选 client/provider 写入 window 变量 `@agent_client`/`@agent_provider`。
...
- 每个 AI window 默认使用两个 pane，也可在设置中启用第三个：
- `agent`：主 AI agent 终端。
- `git`：git/status 或提交相关终端，直接打开 `lazygit`。
- `run`（可选）：测试、dev server、watch 或临时 shell。

这样组织的好处是：

- 每个项目的上下文固定在一个 tmux session 中。
- 每个 agent 的工作环境固定在一个 window 中。
- git 状态和 AI 对话不会互相覆盖；额外命令按需手动分 pane。
...
新窗口按 **Settings → Window & Resize** 生成布局。默认是两 pane、固定 `landscape`：左侧 agent 约 55%，右侧 git 约 45%；**Auto orientation on resize** 默认 OFF，终端变窄或切到竖屏不会自动改朝向。

两种朝向：

```text
纵向 (portrait)                 横向 (landscape，默认)
+-----------------------+      +------------------+------------+
| agent             55% |      | agent        55% | git    45% |
+-----------------------+      |                  |            |
| git               45% |      |                  |            |
+-----------------------+      +------------------+------------+
```
...
- `git`：查看 `git status`、diff、log，或做提交相关操作（直接启动 `lazygit`，无 lazygit 时回退 `git status --short --branch`）。
- `run`（可选）：第三 pane 开启后位于 git 下方，用于测试、dev server、watch 或临时 shell。

新窗口默认是固定 **landscape 模式**，竖屏/resize 不会切换朝向。焦点最后回到 `agent` pane；需要固定 run pane 时打开 **Third run pane**，临时需求仍可用 `prefix -` / `prefix |` 手动分 pane。

### 布局朝向与自动 reflow

自动 reflow 是 opt-in。打开 **Settings → Window & Resize → Auto orientation on resize** 后，agent-tracker 才会按窗口宽高比（终端 cell 高约宽 2 倍，带滞回防抖）决定横/竖；只有朝向真正变化才用 `break-pane -d` 摘下 git/run pane，再 `join-pane` 无损接回，不杀 pane 内进程，并恢复原 active pane。

**手动方向与比例**：Settings 同页的 **Window orientation** 会立刻切换当前窗口的 landscape/portrait，并保存为 auto-resize 关闭时的新窗口默认；若自动开关仍开着，后续跨阈值 resize 仍可再次改变朝向。**Agent / side ratio** 控制主分割（默认 55:45）。同一朝向下手动拖动 pane 后，后台不会再把比例拉回；配置比例只在建窗或横竖方向实际切换时应用。

**可选第三 pane**：打开 **Third run pane** 后，新窗口使用 ai/git/run 三 pane；已有窗口不会自动增删 pane，避免误杀进程。**Git / run ratio** 控制右侧上/下比例（默认 75:25）。
...
### Window & Resize 设置（`alt-s` → Settings → Window & Resize）

- **Auto orientation on resize**（默认 OFF）：只在终端宽高跨过 landscape/portrait 阈值时自动 reflow；关闭时后台绝不改布局。
- **Window orientation**（默认 landscape）：立即切换当前窗口的横/竖布局，并成为 auto-resize 关闭时的新窗口默认方向。
- **Agent / side ratio**（默认 55:45）：主轴上 agent 与 git 侧的建窗/reflow 比例；同朝向手动拖动不受约束。
- **Third run pane**（默认 OFF）：新窗口是否在 git 下方增加 run pane；不自动增删已有窗口的 pane。
- **Git / run ratio**（默认 75:25）：三 pane 模式下右侧 git（上）与 run（下）的比例。
- **Status bar position**（auto/top/bottom）：状态栏位置；`auto` 跟随当前布局方向。

### General 设置补充项（`alt-s` → Settings → General）

布局、resize 与 status 位置已移到独立的 **Settings → Window & Resize**。General 中除 Notifications / Notification grouping / Icon set / Timer timezone 外，还有这些常用项：
...
通过”一个项目一个 tmux session”，AI session 会更稳定地留在对应项目目录下。`prefix ]`（当前目录）/`prefix [`（指定目录）创建的新 window 会从目标路径解析 git root，并把所有布局 pane 都放在项目根目录。
...
默认两 pane 布局把主要工作面分开，同时给 LazyGit 足够宽度：

- AI 输出留在 `agent` pane。
- git 状态和提交操作留在 `git` pane，直接使用 LazyGit 自己的默认视图。
- 测试、server、watch 命令可启用固定第三 run pane，或按需用 `prefix -` / `prefix |` 新开 pane。
...
- **status 错位（per-machine 固定）**：内层（远程机）status 固定**上方**、外层（本地机）固定**下方**，两层错开不叠。这靠**每台机器各自的固定设置**实现（不随尺寸/朝向自动变换）：
  - 本地（外层）机：Window & Resize → Status bar position 设 `bottom`（或 `agent-config.json` 的 `status_position: "bottom"`）——含 ssh 窗、含竖版一律底部。
  - 远程（内层）机：同设为 `top`。
...
- **`agent-config.json` 是 per-machine**：window-title 开关 / `layout_*` / `status_position` 存在 `~/.config/agent-tracker/agent-config.json`，**不在 `~/.hat-config` 路径下、不被 Syncthing 同步**——有意为之，每台机独立设置。跨机想一致需手动对齐。
