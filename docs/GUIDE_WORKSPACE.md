...
**恢复时**：按快照重开 session/window、摆正编号、重建 ai/git/run 三格、在 git 格起 lazygit。**不会自动启动 agent**。但如果该窗口存到了 Claude Code 的 session id，恢复时会在 ai 格**预填**好命令：
...
```bash
d=$(mktemp -d)
TMUX_TMPDIR=$d tmux new-session -d -s clean -x 200 -y 50
TMUX_TMPDIR=$d ~/.hat-config/tmux/scripts/restore_workspace.sh "$(cat ~/.hat-config/state/workspaces/last)"
TMUX_TMPDIR=$d tmux list-windows -a -F '#{window_index} #{window_name} panes=#{window_panes}'
TMUX_TMPDIR=$d tmux kill-server; rm -rf "$d"
```
应看到窗口按快照重建、每个 3 格。
...
```
验收点：窗口数量/顺序/名字与崩溃前一致；repo 窗口是三格 + lazygit 起来；ai 格空 shell，`agent -r` 续对话。
...
| `tmux/scripts/build_agent_layout.sh` | 三格 ai/git/run 布局重建（新建与恢复共用） |
