...
pane    = window 里的一个分屏区域，推荐对应 agent / git / 可选 run
```
...
一个 agent window = 默认两个 pane，可选三个
```
...
- `run`（可选）: 位于 git 下方，运行测试、dev server、watch 或临时 shell。
...
tmux 原生只有一条 status line，不能同时在顶部和底部各放一条完整 status。status line 位置由 **Settings → Window & Resize → Status bar position** 控制，三种模式（默认 auto）：
- `auto`：**跟随当前 active window 的布局朝向**——横向布局 → 底部，纵向布局 → 顶部。布局朝向变化（自动 reflow 或直接改 Window orientation）就等于切换 status 上下位置。
- `top` / `bottom`：固定到该位置，不随布局变。

实现：`tmux/scripts/update_status_position.sh` 读 Window & Resize 设置（`agent tmux status-position`）；auto 时读当前 window 的 `@agent_orientation`，幂等 `set -g status-position`；在布局变换（`reflow_agent_layout.sh`/`build_agent_layout.sh`）、切窗/尺寸变化（`after-select-window`/`window-resized` hook）、`client-resized`/`client-attached` 时调用。非 agent 窗口回退按 client 视觉宽高比（`client_width < client_height*2`）判定。prefix 状态另外叠加到 active pane 标题里，无论 status 在顶还是底都能就近看到。
...
| `prefix ]` | 在当前目录按 Window & Resize 设置新建 2/3-pane agent window（无输入，名字自动） |
...
- 用 `agent` 启动器（见 `docs/GUIDE.md`）会自动建好带 2/3-pane 配置布局、自动命名的工作窗口，通常不必手动建。
...
- **`prefix ]`**：在**当前 pane 目录**按 Window & Resize 设置创建 agent/git（可选 run）布局，不弹任何输入，名字走默认（由 agent-tracker 自动命名为 `项目/标题`）。
...
布局设置集中在 **Settings → Window & Resize**：默认 `landscape`、两 pane、agent/git=55:45，自动方向切换 OFF。**Window orientation** 会立即切换当前窗口，并更新 auto-resize 关闭时的新窗口默认；打开 **Auto orientation on resize** 后，才按窗口宽高比自动切换横向或纵向。
...
Auto orientation 开启后，resize 跨过横/竖阈值才会 reflow——无损重排 git/run pane、不杀进程，并保留当前 active pane。**同一朝向内手动拖动分隔线不会被比例纠偏拉回**；55:45（以及三 pane 的 Git/run 比例）只在建窗或真正切换朝向时应用。

### 直接切换方向与可选第三 pane

在 **Settings → Window & Resize → Window orientation** 按 Enter，可直接在 landscape/portrait 间切换当前窗口；自动开关关闭时，后台不会再改变它。**Third run pane** 默认 OFF，只影响新建窗口；开启后右侧为 git（上）/run（下），默认 75:25，可由 **Git / run ratio** 调整。已有窗口不会自动增删 pane。

### Git pane 里的 lazygit

agent 工作流会在 `git` pane 中直接启动：

```bash
lazygit
```

不附加命令行参数；本机 LazyGit 配置固定为 `gui.screenMode: normal`、`gui.portraitMode: never`，因此窗口尺寸变化时不会自动切成堆叠布局。左侧启用聚焦展开，Files 聚焦时占主要高度，Local branches 和 Commits 各保留约两行内容。
...
- **Other Windows**：聚合**其它 live window** 的 timer，每行带该 window 的**状态图标**（●busy/○idle/?asking/L limited/E error）、**名字**、**目录**（`@agent_dir`）；`Enter` 启停 / `e` 编辑 / `x` 删除作用于所选 timer 的所属 window（跨窗管理）。**已结束（tmux 中已不存在）的 window 的 timer 不显示**。每条仍 fire 到各自 window。
...
tmux server 偶尔会整体崩溃（`[server exited]` / `tmux a: no sessions`），所有 session/window 一次性丢失。这套机制**只记录骨架快照**（哪些 session、第几个 window、叫什么名字、对应哪个 git repo、主布局朝向），崩溃后按快照与当前 Window & Resize 默认重建结构——不保存运行进程，agent 需手动 `agent -r` 续。
...
恢复时每个 repo window 会按当前设置重建 ai/git[/run] 并在 git pane 起 lazygit；ai pane 留空，由你 `agent -r` 续上崩溃前的对话。
...
默认保持横向 55/45，resize 或切竖屏不自动改朝向。需要切到纵向或启用自动切换时，进入 **Settings → Window & Resize** 修改 Window orientation / Auto orientation on resize。
