...
- **Auto del**：**完成后自动删除**（表单手动新建默认 off；`r` 一键 continue@reset、Other/History 栏 `v` 一键复制**默认 on**——一键创建的即用即走，模板真身留在 History）——一次性 timer 用完即删；循环 timer 到 Max exec 后删；`Space`/`y`/`n` 切换
...
**全局** timer 历史模板库：所有 window 用过的 timer 内容跨窗口合并展示（按 `内容+trigger+loop+max+sendEnter` 组合去重、最近使用在前），**持久化**——timer 删了、window 关了、tmux server 重启（window id 变了）历史都还在。注意这与**活动 timer** 相反：活动 timer 绑定 window id，**window 关闭即被自动清理、tmux server 重启后全部作废**（id 空间重置，旧 timer 若保留会串到复用相同 id 的新窗口）；要复用配置从 History `v` 一键取回。
...
典型组合：`prefix t` → `r` 一键（等价 `a` 新建 → Content `continue` / Trigger `reset` / Send Enter on / Auto del on，用完即删）；想每次撞限都自动续，`a` 新建并把 Loop 填 `reset`。

### `[L]` limited 状态（额度满，Claude + Codex 均支持）

Claude 会话最近一个 turn 死在 429 rate_limit 且重置时刻未到、或 Codex 会话 rollout 最新用量快照显示 5h/周窗口用满（`used_percent>=95%`）时，窗口名前缀变 **`[L]`**（取代 `[I]`/`[?]`），Window Nav 显示红色 `L` activity 图标；daemon 侧视同 asking，首次未读时抬起 🔔 + 通知并进入「需要处理」。用户进入看过后 acknowledge 清掉 🔔，窗口即离开「需要处理」，但额度恢复前仍保留 `L` 状态。重置时刻同时写进窗口选项 `@agent_limit_reset_at`（epoch 秒），可供脚本读取。重置时刻一过（或新 turn 跑通）自动恢复正常状态。`prefix t` 的 `reset` 触发/`r` 一键续同样对 Codex 窗口生效。
