...
var statusMemoryCacheRefreshScript = func() string {
	return filepath.Join(os.Getenv("HOME"), ".hat-config", "tmux", "tmux-status", "mem_usage_cache.py")
}
...
func formatRightStatusSegments(statusBG string, segments []statusSegment) string {
	...
	separator := activeIconSet().SepRight
	rightCap := "█"
	prevBG := statusBG
...
func loadCPUStatusLabel() string {
	...
	return fmt.Sprintf(" %s %s ", activeIconSet().CPU, formatUsagePercent(total))
}
...
func loadNetworkStatusLabel() string {
	...
	if previous.Interface == iface && previous.SampledAt > 0 && now > previous.SampledAt && current.InBytes >= previous.InBytes && current.OutBytes >= previous.OutBytes {
		...
		if seconds >= 0.25 {
			...
			rate = fmt.Sprintf(" %s ↓%s ↑%s ", activeIconSet().Network, formatByteRate(down), formatByteRate(up))
		}
	}
	...
	if rate == " ↓-- ↑-- " {
		return fmt.Sprintf(" %s ↓-- ↑-- ", activeIconSet().Network)
	}
	return rate
}
...
func loadMemoryStatusLabel(paneID string) string {
	...
	return fmt.Sprintf(" %s %s ", activeIconSet().Memory, value)
}
...
func loadMemoryTotalsStatusSegments(args tmuxRightStatusArgs) []statusSegment {
	...
	if value := strings.TrimSpace(cache.Window[windowKey]); value != "" {
		segments = append(segments, statusSegment{FG: "#eceff4", BG: "#4c566a", Text: fmt.Sprintf(" %s %s ", activeIconSet().Window, value)})
	}
	if value := strings.TrimSpace(cache.Session[strings.TrimSpace(args.SessionName)]); value != "" {
		segments = append(segments, statusSegment{FG: "#eceff4", BG: "#434c5e", Text: fmt.Sprintf(" %s %s ", activeIconSet().Session, value)})
	}
	if value := strings.TrimSpace(cache.Total); value != "" {
		segments = append(segments, statusSegment{FG: "#eceff4", BG: "#3b4252", Text: fmt.Sprintf(" %s %s ", activeIconSet().Total, value)})
	}
	return segments
}
...
func loadTodosStatusLabel(windowID string) string {
	...
	if !statusRightModuleEnabled(statusRightModuleTodoPreview) {
		return fmt.Sprintf(" %s %d ", activeIconSet().Todos, count)
	}
	title := firstOpenStatusTodoTitle(items)
	if title != "" {
		return fmt.Sprintf(" %s %d %s ", activeIconSet().Todos, count, truncate(title, statusTodoMaxChars()))
	}
	return fmt.Sprintf(" %s %d ", activeIconSet().Todos, count)
}
...
func loadFlashMoeStatusSegment() (statusSegment, bool) {
	...
	case "prefill":
		...
		label := fmt.Sprintf(" %s prefill ", activeIconSet().FlashMoe)
		if promptTokens != "" && promptTokens != "0" {
			label = fmt.Sprintf(" %s prefill:%s ", activeIconSet().FlashMoe, promptTokens)
		}
		return statusSegment{FG: "#1d1f21", BG: "#ebcb8b", Text: label, Bold: true}, true
	case "gen":
		tokS := strings.TrimSpace(values["tok_s"])
		label := fmt.Sprintf(" %s gen ", activeIconSet().FlashMoe)
		if tokS != "" && tokS != "0.00" {
			label = fmt.Sprintf(" %s %s tok/s ", activeIconSet().FlashMoe, tokS)
		}
...
func defaultStatusRightModuleEnabled(module string) bool {
	switch module {
	case statusRightModuleCPU, statusRightModuleNetwork, statusRightModuleMemory, statusRightModuleTodoPreview, statusRightModuleTodos, statusRightModuleFlashMoe, statusRightModuleHost:
		return true
...
func isValidStatusRightModule(module string) bool {
	switch module {
	case statusRightModuleCPU, statusRightModuleNetwork, statusRightModuleMemory, statusRightModuleMemoryTotals, statusRightModuleTodoPreview, statusRightModuleTodos, statusRightModuleFlashMoe, statusRightModuleHost:
		return true
	default:
		return false
	}
...
func (cfg *statusRightConfig) isDefault() bool {
	...
	return cfg.CPU == nil && cfg.Network == nil && cfg.Memory == nil && cfg.MemoryTotals == nil && cfg.TodoPreview == nil && cfg.Todos == nil && cfg.FlashMoe == nil && cfg.Host == nil
}
