...
// trackerDialTimeout bounds a single request/response exchange with the daemon
// over the unix socket, so a wedged daemon can't hang the palette indefinitely.
const trackerDialTimeout = 5 * time.Second
...
// trackerSocketPath returns the daemon's unix socket path.
func trackerSocketPath() string {
	return paths.SocketPath()
}

// trackerLoadState connects to the daemon and returns the first state envelope.
func trackerLoadState(client string) (*ipc.Envelope, error) {
	...
	defer conn.Close()
	_ = conn.SetDeadline(time.Now().Add(trackerDialTimeout))
...
// sendTrackerCommand sends a command envelope to the daemon and waits for ack.
