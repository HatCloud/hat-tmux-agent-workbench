...
// handleMouse: single-line rows in a table body starting at Y=4 (title=0,
// meta=1, blank=2, table header=3). Only active when no modal is open.
func (m *activityMonitorBT) handleMouse(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	if m.confirmKillPID != 0 || len(m.copyOptions) > 0 {
		return m, nil
	}
	...
	case tea.MouseButtonLeft:
		...
		m.selectedRow = idx
		m.selectedPID = m.rows[idx].PID
		return m.updateNormal("enter")
	}
	return m, nil
}
...
func (m *activityMonitorBT) renderFooter(width int) string {
	...
	if m.showAltHints {
		...
		footer = pickRenderedShortcutFooter(width, renderSegments,
			[][2]string{{"j/k", "move"}, {"n/i", "sort"}, {"u/e", "net"}, {"a", "scope"}, {"l", "follow"}, {"r", "reverse"}, {"Enter", "tmux"}, {"d", "kill"}, {"y", "copy"}, {"Esc", "back"}, {footerHintToggleKey, "more"}},
			[][2]string{{"j/k", "move"}, {"n/i", "sort"}, {"u/e", "net"}, {"a", "scope"}, {"Enter", "tmux"}, {"d", "kill"}, {"y", "copy"}, {"Esc", "back"}, {footerHintToggleKey, "more"}},
