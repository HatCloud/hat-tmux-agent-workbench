...
type todoPanelModel struct {
	...
	// captured by renderList / renderPane each frame; MouseMsg uses these to
	// hit-test panes and map (X, Y) → entry index.
	paneHit map[todoPanelPane]todoPaneHitRegion
}

type todoPaneHitRegion struct {
	x0, x1      int   // absolute X range (inclusive, exclusive)
	firstRowY   int   // absolute Y of first entry line
	lineToEntry []int // for each visible line in body: entry index (or -1 for padding)
}
...
func (m *todoPanelModel) updateList(key string) (tea.Model, tea.Cmd) {
	...
	case "p":
		...
		dupScope := todoScopeWindow
		dupScopeID := m.windowID
		if m.focusedPane == todoPanelPaneGlobal {
			dupScope = todoScopeGlobal
			dupScopeID = "global"
		}
		if err := addTmuxTodo(dupScope, dupScopeID, entry.Title); err != nil {
			m.setStatus(err.Error(), 1500*time.Millisecond)
		} else {
			m.reloadEntries()
			m.setStatus("Duplicated", 1500*time.Millisecond)
		}
	case "y":
		entry, ok := m.selectedEntry(m.focusedPane)
		if !ok {
...
// buildLineToEntry mirrors renderPane's offset+visibleRows loop to produce a
// line-index → entry-index mapping. -1 marks blank/padding lines.
func (m *todoPanelModel) buildLineToEntry(pane todoPanelPane, width, visibleRows int) []int {
	entries := m.visibleEntries(pane)
	if len(entries) == 0 || visibleRows <= 0 {
		return nil
	}
	selected := m.selectedIndex(pane)
	offset := stableListOffset(m.selectedOffset(pane), selected, visibleRows, len(entries))
	out := make([]int, 0, visibleRows)
	usedRows := 0
	for idx := offset; usedRows < visibleRows; idx++ {
		if idx >= len(entries) {
			break
		}
		lines := m.renderTodoEntryLines(entries[idx], width, false)
...
func (m *todoPanelModel) handleMouse(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	if m.mode != todoPanelModeList {
		return m, nil
	}
	...
	for pane, hit := range m.paneHit {
		if msg.X < hit.x0 || msg.X >= hit.x1 {
			continue
		}
		rel := msg.Y - hit.firstRowY
		if rel < 0 || rel >= len(hit.lineToEntry) {
			continue
		}
		entryIdx := hit.lineToEntry[rel]
		if entryIdx < 0 {
			continue
		}
		m.setFocusedPane(pane)
		m.setSelectedIndex(pane, entryIdx)
...
func (m *todoPanelModel) renderList(w, h int) string {
	...
	// Record hit regions: Padding(0,1) shifts content by 1 col; body starts at Y=3
	// (header, meta, blank). Pane header (label+meta) is row 0, blank is row 1,
	// so entries start at Y = paneY0 + 2.
	m.paneHit = map[todoPanelPane]todoPaneHitRegion{
		todoPanelPaneWindow:     {x0: 1, x1: 1 + leftW, firstRowY: 3 + 2, lineToEntry: m.buildLineToEntry(todoPanelPaneWindow, leftW, maxInt(1, upperLeftH-2))},
		todoPanelPaneAllWindows: {x0: 1, x1: 1 + leftW, firstRowY: 3 + upperLeftH + 1 + 2, lineToEntry: m.buildLineToEntry(todoPanelPaneAllWindows, leftW, maxInt(1, lowerLeftH-2))},
		todoPanelPaneGlobal:     {x0: 1 + leftW + 1, x1: 1 + leftW + 1 + rightW, firstRowY: 3 + 2, lineToEntry: m.buildLineToEntry(todoPanelPaneGlobal, rightW, maxInt(1, contentH-2))},
	}
...
func (m *todoPanelModel) renderFooter(w int) string {
	...
	if m.showAltHints {
		...
		footer = pickRenderedShortcutFooter(contentWidth, renderSegments,
			[][2]string{{"j/k", "move"}, {"Enter", "goto"}, {"Ctrl-J/K", "reorder"}, {"n/i", "column"}, {"Tab", "win↔all"}, {"N/I", "scope"}, {"Space", "toggle"}, {"a", "add"}, {"e", "edit"}, {"p", "dup"}, {"y", "copy text"}, {"d", "delete"}, {"1/2/3", "priority"}, {"c", "completed"}, {"Esc", "close"}, {footerHintToggleKey, "more"}},
			[][2]string{{"j/k", "move"}, {"Enter", "goto"}, {"Ctrl-J/K", "reorder"}, {"n/i", "col"}, {"Tab", "win↔all"}, {"N/I", "scope"}, {"Space", "toggle"}, {"a", "add"}, {"e", "edit"}, {"p", "dup"}, {"y", "copy"}, {"d", "del"}, {"c", "done"}, {"Esc", "close"}, {footerHintToggleKey, "more"}},
			[][2]string{{"j/k", "move"}, {"Enter", "goto"}, {"Ctrl-J/K", "reorder"}, {"n/i", "col"}, {"Tab", "win↔all"}, {"N/I", "scope"}, {"Space", "toggle"}, {"a", "add"}, {"e", "edit"}, {"p", "dup"}, {"y", "copy"}, {"d", "del"}, {"Esc", "close"}, {footerHintToggleKey, "more"}},
			[][2]string{{"j/k", "move"}, {"Enter", "goto"}, {"n/i", "col"}, {"Tab", "↔all"}, {"N/I", "scope"}, {"Space", "toggle"}, {"a", "add"}, {"e", "edit"}, {"p", "dup"}, {"y", "copy"}, {"d", "del"}, {"Esc", "close"}, {footerHintToggleKey, "more"}},
			[][2]string{{"j/k", "move"}, {"Enter", "goto"}, {"n/i", "col"}, {"Tab", "↔all"}, {"N/I", "scope"}, {"a", "add"}, {"e", "edit"}, {"p", "dup"}, {"d", "del"}, {"Esc", "close"}, {footerHintToggleKey, "more"}},
