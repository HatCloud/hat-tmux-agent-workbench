...
func main() {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		target, err := resolveContext(input.TmuxID, autodetectContext)
		if err != nil {
			return nil, nil, err
		}
...
// resolveContext resolves the tmux target from an explicit tmux_id, or falls
// back to autodetect when tmux_id is empty (so callers that cannot inject the
// coordinate — e.g. a bare MCP call — still work inside tmux).
func resolveContext(tmuxID string, autodetect func() (tmuxContext, error)) (tmuxContext, error) {
	if strings.TrimSpace(tmuxID) == "" {
		return autodetect()
	}
	return determineContext(tmuxID)
}
...
func autodetectContext() (tmuxContext, error) {
	// Bound the whole autodetect: a hung tmux/ps must not block the MCP call,
	// which runs autodetect before the handler's own timeout applies.
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()

	pane := strings.TrimSpace(os.Getenv("TMUX_PANE"))
	if pane != "" {
		out, err := exec.CommandContext(ctx, "tmux", "display-message", "-p", "-t", pane, "#{session_id}:::#{window_id}:::#{pane_id}").CombinedOutput()
		if err == nil {
	...
	ppid := os.Getppid()
	ttyOut, err := exec.CommandContext(ctx, "ps", "-o", "tty=", "-p", fmt.Sprint(ppid)).CombinedOutput()
	if err == nil {
		tty := strings.TrimSpace(string(ttyOut))
		if tty != "" && tty != "?" {
			out, err := exec.CommandContext(ctx, "tmux", "display-message", "-p", "-c", "/dev/"+tty, "#{session_id}:::#{window_id}:::#{pane_id}").CombinedOutput()
			if err == nil {
				parts := strings.Split(strings.TrimSpace(string(out)), ":::")
				if len(parts) == 3 {
					return tmuxContext{SessionID: strings.TrimSpace(parts[0]), WindowID: strings.TrimSpace(parts[1]), PaneID: strings.TrimSpace(parts[2])}, nil
